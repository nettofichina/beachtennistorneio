    ...
    jogosManuais.forEach((jogo, index) => {
        ...
        if (melhorDeTresCheckbox.checked) {
            jogoDiv.innerHTML = `
                ...
                <input type="text" placeholder="Placar (ex: 6-1)" data-jogo-index="${index}" ${jogo.submetido ? 'disabled' : ''}>
...
document.getElementById('sortearDuplasManuais').addEventListener('click', () => {
    ...
    atualizarJogos();
    dadosAlterados = true; // Marca que os dados foram alterados
    alert('Duplas sorteadas aleatoriamente!');
});
...
function submeterPlacar(event) {
    ...
    if (/^\d+-\d+$/.test(placar)) {
        ...
        if (!jogo.submetido) {
            ...
            if (melhorDeTresCheckbox.checked) {
                // Verificar se a dupla já ganhou dois sets para desabilitar o terceiro set
...
document.getElementById('salvarTorneioManual').addEventListener('click', () => {
...
document.getElementById('voltarParaHome').addEventListener('click', () => {
    if (dadosAlterados) {
